    ...
    return (
      <div className="text-center py-16 bg-white rounded-2xl shadow-lg">
        ...
        <p className="text-gray-600 mb-6 max-w-md mx-auto">
          We couldn't find any doctors matching your criteria. Try adjusting your search filters or clear all filters.
        </p>
        <button
          ...
          <div className="flex flex-col md:flex-row gap-6">
            ...
            <div className="flex-1">
              <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                <div className="flex-1">
                  ...
                  <div className="flex flex-wrap gap-4 mb-4">
                    <div className="flex items-center gap-2">
                      <div className="flex items-center">
                        {[...Array(5)].map((_, i) => (
                          <svg key={i} className={`h-4 w-4 ${i < Math.floor(doctor.rating) ? "text-yellow-400 fill-yellow-400" : "text-gray-300"}`} fill="currentColor" viewBox="0 0 20 20">
                  ...
                  <div className="flex flex-wrap gap-2 mb-4">
                    {Array.isArray(doctor.tags) && doctor.tags.map((tag, i) => (
                      <span key={i} className="px-2 py-1 bg-gray-100 text-gray-700 text-xs rounded-full">
                        {tag}
                      </span>
                    ))}
                    {Array.isArray(doctor.languages) && doctor.languages.map((lang, i) => (
                      lang && (
                        <span key={`lang-${i}`} className="px-2 py-1 bg-blue-100 text-blue-700 text-xs rounded-full">
                          {lang}
                        </span>
                      )
                    ))}
                ...
                <div className="flex-shrink-0 text-right">
                  <div className="text-3xl font-bold text-green-600">{doctor.fee} ৳</div>
                  <p className="text-sm text-gray-500 mb-4">consultation fee</p>
                  <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full ${doctor.isOnline ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-800"}`}>
                    <div className={`h-2 w-2 rounded-full ${doctor.isOnline ? "bg-green-500" : "bg-gray-400"}`}></div>
                    <span className="text-sm font-medium">{doctor.isOnline ? "Online Now" : "Offline"}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-wrap gap-3 mt-6">
                ...
                >
                  <svg className={`w-5 h-5 ${favorites.includes(doctor._id) ? "fill-red-400" : ""}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
