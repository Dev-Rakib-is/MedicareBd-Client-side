...
export default function DoctorRegistrationForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.workingDays.length === 0) return setError("Please select at least one working day");
    if (!formData.workingHours.from || !formData.workingHours.to) return setError("Please select working hours");
    if (!formData.registrationNumber) return setError("Registration number is required");
    ...
    try {
      ...
      formDataToSend.append("workingHours[from]", formData.workingHours.from);
      formDataToSend.append("workingHours[to]", formData.workingHours.to);
      ...
      setFormData({
        name: "", email: "", password: "", specialization: "", registrationNumber: "", fee: "",
        workingDays: [], workingHours: { from: "", to: "" }, photo: null, termsAccepted: false,
      });
      setPreview(null);
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
      <div className="w-full max-w-2xl bg-white rounded-2xl shadow-lg p-8">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-3">Doctor Registration</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <input type="text" name="registrationNumber" placeholder="Registration Number" value={formData.registrationNumber} onChange={handleChange} required
                className="w-full p-4 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:border-green-500 focus:ring-2 focus:ring-green-200 transition-all" />
              <input type="number" name="fee" placeholder="Consultation Fee (USD)" value={formData.fee} onChange={handleChange} required min="0" step="0.01"
                className="w-full p-4 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:border-green-500 focus:ring-2 focus:ring-green-200 transition-all" />
            </div>
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              <label className="block text-sm font-medium text-gray-700">Select Working Days *</label>
              <div className="grid grid-cols-7 gap-2">
                {DAYS.map((day) => (
                  <button type="button" key={day.value} onClick={() => toggleDay(day.value)}
                    className={`py-3 rounded-lg border transition-all ${formData.workingDays.includes(day.value)
                        ? "bg-green-500 text-white border-green-500" : "bg-gray-50 text-gray-600 border-gray-200 hover:border-green-300"}`}>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="time" name="workingHours.from" value={formData.workingHours.from} onChange={handleChange} required
                    className="w-full p-4 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:border-green-500 focus:ring-2 focus:ring-green-200 transition-all" />
                  <p className="text-xs text-gray-500 mt-1">Format: 9:00, 09:00, 5:00 PM, 17:00</p>
                </div>
                <div>
                  <label className="block text-sm text-gray-600 mb-2">To</label>
                  <input type="time" name="workingHours.to" value={formData.workingHours.to} onChange={handleChange} required
                    className="w-full p-4 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:border-green-500 focus:ring-2 focus:ring-green-200 transition-all" />
                </div>
              </div>
            </div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-4">
              ...
              {preview && <img src={preview} alt="Preview" className="w-20 h-20 rounded-full object-cover border-2 border-green-200" />}
            </div>
          </div>
          ...
          <div className="space-y-6 pt-4">
            <label className="flex items-start gap-3 cursor-pointer">
              <div className="relative mt-1">
                ...
                <div className={`w-5 h-5 rounded border transition-all flex items-center justify-center ${formData.termsAccepted ? "bg-green-500 border-green-500" : "border-gray-300 hover:border-green-400"}`}>
                  {formData.termsAccepted && <span className="text-white font-bold text-sm">✓</span>}
                </div>
              </div>
              <span className="text-sm text-gray-600">
                I agree to the <Link to="/terms" className="text-green-600 hover:text-green-700 font-medium">Terms & Conditions</Link> and{" "}
                <Link to="/privacy-policy" className="text-green-600 hover:text-green-700 font-medium">Privacy Policy</Link>
              </span>
            </label>
